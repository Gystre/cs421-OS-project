...
/*
As abstracted as I could make the print statement. Use this whenever you need to print something

ex. kprintf("%s what the? %s %d %x", "bruh", "nah", 1234567890, 0xABCD);
*/
void kprintf(char *format, ...)
{
    ...
    while (format[i] != '\0')
    {
        if (format[i] == '%')
        {
            ...
            {
            case 'c':
                // char is promoted to int when passed through ellipsis
            ...
            case 'd':
                ...
                break;
            case 'x':
                kprint_hex(va_arg(args, int));
                break;
            default:
                // If an unsupported format specifier is found, just print the '%'
                kprint_char('%');
